...
def buy():
    ...
    if request.method =="POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date, buy_sell) VALUES (?, ?, ?, ?, ?, ?)", session["user_id"], symbolInfo["symbol"], shares, symbolInfo["price"], date, "Bought")
...
def history():
    ...
    return render_template("history.html", transactions=transactionInfo)
...
def sell():
    ...
    if request.method == "POST":
        ...
        flash(str(sharesToSell) + " " + symbolInfo["symbol"] + " " + "share(s) sold for $"+ "{:.2f}".format(symbolInfo["price"] * sharesToSell) +". Your account balance is $" + "{:.2f}".format(newCashAmount))
